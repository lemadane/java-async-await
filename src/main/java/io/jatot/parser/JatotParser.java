...
public final class JatotParser {
    ...
    private String parseDottedName() {
        ...
        while (match(TokenType.DOT)) {
            ...
            sb.append(".").append(consume(TokenType.IDENTIFIER, "Expected identifier after '.'."));
        }
        return sb.toString();
    }
    ...
    private Member parseMember(String enclosingClassName) {
        ...
        TypeNode type;
        String name;
        if (check(TokenType.IDENTIFIER) && (lookAhead(1).type() == TokenType.ASSIGN || lookAhead(1).type() == TokenType.SEMICOLON)) {
        ...
        if (match(TokenType.LEFT_PAREN)) {
            ...
            return new MethodDecl(modifiers, typeParams, type, name, parameters, throwsList, body);
        } else {
            // Field
